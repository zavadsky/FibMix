...
class UniversalCode {
public:
    UniversalCode(string name){buffer = (unsigned char*)calloc(50000000,1);buffer32=(uint32_t*)buffer;c_name=name;};
...
class FibMixTCode: public UniversalCode {
public:
    FibMixTCode(WordBasedText*);
private:
    vector<int> digits;
};

class Fib3Code: public UniversalCode {
public:
    Fib3Code();
};

class Fib2Code: public UniversalCode {
public:
    Fib2Code();
};

class FibUCode: public UniversalCode {
public:
    FibUCode(int);
private:
    int d;
